...
# ========== TAB 3: BRACKETS HORIZONTALES (CORREGIDO) ==========
with tab3:
    ...
    if not df_brackets.empty:
        ...
        if not df_cat.empty:
            ...
            html_brackets = """
            <div style="overflow-x: auto; overflow-y: visible; margin: 20px 0; padding: 10px 0;">
                <div style="
                    display: flex;
                    flex-direction: row;
                    flex-wrap: nowrap;
            ...
            for ronda in rondas:
                ...
                for _, partido in df_ronda.iterrows():
                    # Determinar clases para ganadores
                    c1_winner = partido['Ganador'] == partido['Competidor1'] and partido['Ganador'] != ""
                    c2_winner = partido['Ganador'] == partido['Competidor2'] and partido['Ganador'] != ""
                    
                    # Estilos para competidores
                    c1_style = "background: rgba(255,215,0,0.15); border-left: 4px solid gold;" if c1_winner else "border-left: 4px solid #ff2b2b;"
                    c2_style = "background: rgba(255,215,0,0.15); border-left: 4px solid gold;" if c2_winner else "border-left: 4px solid #1e90ff;"
                    ...
                    # Badge BYE
                    bye_badge = '<span style="position:absolute; top:-8px; left:5px; background:gold; color:black; font-size:0.6rem; padding:2px 6px; border-radius:10px; font-weight:bold;">⭐ BYE</span>' if partido['Competidor1'] == "BYE" or partido['Competidor2'] == "BYE" else ""
                    
                    # Línea conectora (solo si no es la última ronda)
                    conector = f'<div style="position:absolute; top:50%; right:-40px; width:40px; height:2px; background:linear-gradient(90deg, #ff2b2b, #666); z-index:1;"></div>' if ronda < total_rondas else ""
                    ...
                    ">
                        ...
                        <div style="padding: 8px 12px; border-bottom: 1px solid #333; {c1_style}">
                            <div style="display:flex; flex-direction:column;">
                                <span style="font-size:0.9rem; color:#fff; font-weight:500;">{n1}</span>
                                <span style="font-size:0.65rem; color:#888;">{d1}</span>
                            </div>
                        </div>
                        
                        <div style="padding: 8px 12px; {c2_style}">
                            <div style="display:flex; flex-direction:column;">
                                <span style="font-size:0.9rem; color:#fff; font-weight:500;">{n2}</span>
                                <span style="font-size:0.65rem; color:#888;">{d2}</span>
                            </div>
                        </div>
                    </div>
                    """
                
                # Cerrar columna de ronda
                html_brackets += "</div>"
            
            # Cerrar contenedor principal
            html_brackets += "</div></div>"
