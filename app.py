...
INSCRIPCIONES_SHEET = "https://docs.google.com/spreadsheets/d/1hFlkSSPWqoQDSjkiPV5uaIIx-iHjoihLg2yokDJm-4E/edit?gid=123456789"  # Ajusta este ID
    ...
    with tab3:
        ...
        if st.session_state.get('is_admin', False):
            ...
            if not inscriptions_df.empty:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    st.metric("Total Inscritos", len(inscriptions_df))
                    st.metric("Individuales", len(inscriptions_df[inscriptions_df['Tipo_Inscripcion'] == 'individual']))
                    st.metric("Colectivas", len(inscriptions_df[inscriptions_df['Tipo_Inscripcion'] == 'colectivo']))
                ...
                with st.expander("📋 LISTA COMPLETA DE INSCRITOS", expanded=False):
                    display_cols = ['Nombre_Completo', 'Categoria', 'Dojo', 'Estado_Pago', 'Fecha_Inscripcion']
                ...
                if not inscritos_categoria.empty:
                    ...
                    with st.expander("Ver inscritos", expanded=False):
                        for _, row in inscritos_categoria.iterrows():
                            st.markdown(f"**{row['Nombre_Completo']}** - {row['Dojo']}")
...
# --- 9. MANEJO DE EVENTOS DE VOTACIÓN ---
# Esto manejaría los votos desde JavaScript (necesitarías configurar Streamlit para esto)
# Por ahora, dejamos la función de votación tradicional
